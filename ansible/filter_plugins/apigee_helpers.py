    ...
    for app in apps_list:
        ...
        for cred in credentials:
            ...
            for product in api_products:
                ...
                company_exists = "companyName" in app
                developer_exists = "developerId" in app
                ...
                result[api_product].append(
                    {
                        ...
                        "owner": owner,
                        "ownerEndpoint": (
                            "companies" if company_exists else "developers"
                        ),
...
def apigee_remove_proxy_from_product(product: dict, proxy_to_remove):
    product["proxies"] = [p for p in product.get("proxies", []) if p != proxy_to_remove]
    return product
...
def apigee_team_to_admin(team):
    return next(
        (
            attr.get("value")
            for attr in team["attributes"]
            if attr["name"] == "ADMIN_EMAIL"
        ),
        None,
    )
...
def apigee_teams_to_members(teams: List[dict]):
    ...
    return {team["owner"]: team.get("members", []) for team in teams}


def apigee_product_developers(
    product_app_map: dict,
    dev_id_to_email: dict,
    teams_map: dict,
    ...
    result = {key: sorted(apps, key=lambda x: x["app"]) for key, apps in result.items()}
